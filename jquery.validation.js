/*
	...
	Ad esempio è possibile accettare solo lettere o solo numeri o controlli più complessi.
	...
	gestire eventi quando la validazione è completa.
	...
	Non è una cosa obbligatoria, ma permetterebbe a molta più gente di trovare questo
	plugin (e magari altri che svilupperò) ed utilizzarli per rendere più facile la
	vita dei webmaster.
...
jQuery.fn.validation = function( cfg ) {
	...
	cfg.empty			= cfg.empty || "si";			// Può contenere il valore nullo?
	...
	function validateChar(key) {
		...
		else if ( cfg.type == 'lowers' )		return ( keyL(key) || keyAdd(key, "òàùèéì") );	// [a-zòàùèéì]
		else if ( cfg.type == 'upper' )			return keyU(key);					// [A-Z]
		else if ( cfg.type == 'uppers' )		return ( keyU(key) || keyAdd(key, "òàùèéì") );	// [A-Zòàùèéì]
		else if ( cfg.type == 'alpha' )			return ( keyL(key) || keyU(key) );	// [a-zA-Z]
		else if ( cfg.type == 'alphas' )		return ( keyL(key) || keyU(key) || keyAdd(key, "òàùèéì") );	// [a-zA-Zòàùèéì]
		else if ( cfg.type == 'lowerInt' )		return ( keyL(key) || keyN(key) );	// [a-z0-9]
		else if ( cfg.type == 'lowersInt' )		return ( keyL(key) || keyN(key) || keyAdd(key, "òàùèéì") );	// [a-z0-9òàùèéì]
		else if ( cfg.type == 'upperInt' )		return ( keyU(key) || keyN(key) );	// [A-Z0-9]
		else if ( cfg.type == 'uppersInt' )		return ( keyU(key) || keyN(key) || keyAdd(key, "òàùèéì") );	// [A-Z0-9òàùèéì]
		else if	( cfg.type == 'alphaInt')		return ( keyL(key) || keyU(key) || keyN(key) );	// [a-zA-Z0-9]
		else if	( cfg.type == 'alphasInt')		return ( keyL(key) || keyU(key) || keyN(key) || keyAdd(key, "òàùèéì") );	// [a-zA-Z0-9òàùèéì]
	...
	// Prende in input un codice di carattere ed una stringa. Se il carattere è presente in tale
	...
	// caratteri accettati ed un ulteriore controllo sul "quando" è possibile utilizzare determinati
	// catatteri. Ed non è possibile utilizzare 2 volte la "@".
	function keyMail(k) {
		// Controllo formale sul range di caratteri accettati.
		if ( keyL(k) || keyU(k) || keyN(k) || keyAdd(k, "-_.@") ) {			
	
			// Non si può iniziare con un carattere speciale:
			//       .          -          _          @
			if ( ( k == 46 || k == 45 || k == 95 || k == 64 ) && cfg.OBJ.value.length == 0 ) return false;
			
			// Non si può iniziare con un numero:
//			if ( keyN(k) && cfg.OBJ.value.length == 0 ) return false;
			
			// Non può esserci un carattere speciale dopo un punto:
			if ( ( k == 46 || k == 45 || k == 95 || k == 64 ) && cfg.OBJ.value[cfg.OBJ.value.length-1] == "." ) return false;
			
			// Non può esserci un punto o una chiocciola dopo un carattere speciale:
			...
			// Controlli quando è già presente una chiocciola (@)
			if ( cfg.OBJ.value.indexOf("@") != -1 ) {
				...
				// Non si può iniziare con un carattere speciale appena dopo la chiocciola:
			...
			} // Fine controlli quando è già presente una chiocciola.
